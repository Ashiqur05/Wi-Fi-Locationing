...
wifi_data_zero_V_for_Floor_B1 <- subset(wifi_data_zero_V, 
                                     select = -c(USERID, 
                                                 PHONEID, 
                                                 TIMESTAMP,
                                                 LONGITUDE,
...
wifi_validation_zero_V_for_Floor_B1 <- subset(wifi_validation_zero_V, 
                                           select = -c(PHONEID, 
                                                       TIMESTAMP,
                                                       LONGITUDE,
                                                       LATITUDE))
...
wifi_data_traning_B1<-wifi_data_zero_V_for_Floor_B1 %>%
  filter(BUILDINGID==1)
...
wifi_data_validation_B1<-wifi_validation_zero_V_for_Floor_B1 %>%
  filter(BUILDINGID==1)
...
postResample(knnPredict_validation_b1_F, wifi_data_validation_B1$FLOOR)
...
svmFit_b1 <- train(FLOOR ~ ., data = trainSet_B1, 
                   method = "svmLinear", 
                   trControl = ctrl, 
                   #preProc = c("center", "scale"),#???????Warning messages:
                   tuneLength = 2)
#Output of kNN fit
svmFit_b1
#This model seems to be really good as  accuracy and kappa  are almost 1
...
wifi_data_zero_V_for_Floor_B2 <- subset(wifi_data_zero_V, 
                                        select = -c(USERID, 
                                                    PHONEID, 
                                                    TIMESTAMP,
                                                    LONGITUDE,
...
wifi_validation_zero_V_for_Floor_B2 <- subset(wifi_validation_zero_V, 
                                              select = -c(PHONEID, 
                                                          TIMESTAMP,
                                                          LONGITUDE,
                                                          LATITUDE))
...
wifi_data_traning_B2<-wifi_data_zero_V_for_Floor_B2 %>%
  filter(BUILDINGID==2)
...
wifi_data_validation_B2<-wifi_validation_zero_V_for_Floor_B2 %>%
  filter(BUILDINGID==2)
...
postResample(svmPredict_validation_b2_F, wifi_data_validation_B2$FLOOR)#?? low accuracy
###################  SVM_BUILDING_2  #############################################
...
############################################################## BUILDING 3 #####################################################
wifi_data_zero_V_for_Floor_B3 <- subset(wifi_data_zero_V, 
                                        select = -c(USERID, 
                                                    PHONEID, 
                                                    TIMESTAMP,
                                                    LONGITUDE,
...
wifi_validation_zero_V_for_Floor_B3 <- subset(wifi_validation_zero_V, 
                                              select = -c(PHONEID, 
                                                          TIMESTAMP,
                                                          LONGITUDE,
                                                          LATITUDE))
...
wifi_data_traning_B3<-wifi_data_zero_V_for_Floor_B3 %>%
  filter(BUILDINGID==3)
#NO data are available in floor 5 so it gives an error  so to remove this error bellow code has been  used
wifi_data_traning_B3$FLOOR <- as.character(wifi_data_traning_B3$FLOOR)
wifi_data_traning_B3$FLOOR <- as.factor(wifi_data_traning_B3$FLOOR)

wifi_data_validation_B3<-wifi_validation_zero_V_for_Floor_B3 %>%
  filter(BUILDINGID==3)
wifi_data_validation_B3$FLOOR <- as.character(wifi_data_validation_B3$FLOOR)
wifi_data_validation_B3$FLOOR <- as.factor(wifi_data_validation_B3$FLOOR)

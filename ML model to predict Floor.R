...
knnPredict_validation_F <- predict(knnFit_F,newdata = wifi_data_validation)
validation_knnCM_F<-confusionMatrix(knnPredict_validation_F,wifi_data_validation$FLOOR)
...
set.seed(123)
ctrl <- trainControl(method="cv",number = 5) 
SVMFit_F <- train((FLOOR ~ .), data = trainSet, method = "svmLinear", trControl = ctrl, tuneLength = 2,preProcess="zv")
SVMFit_F
...
SVMPredict_validation_F <- predict(SVMFit_F,newdata = wifi_data_validation)
validation_SVMCM_F<-confusionMatrix(SVMPredict_validation_F,wifi_data_validation$FLOOR)
postResample(SVMPredict_validation_F, wifi_data_validation$FLOOR)
...
RFFit_F <- train((FLOOR ~ .), data = SampleTrainFloor, method = "rf", trControl = ctrl, tuneLength = 2,tuneGrid=RFGrid,preProcess="zv")
RFFit_F
...
RFPredict_validation_F <- predict(RFFit_F,newdata = wifi_data_validation)
validation_RFCM_F<-confusionMatrix(RFPredict_validation,wifi_data_validation$FLOOR)
postResample(RFPredict_validation_F, wifi_data_validation$FLOOR)
#################################################  RENDOM FOREST ############################################################